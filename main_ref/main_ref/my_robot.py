...
class MyRobot(Robot):
    ...
    def _tick_odometry(self):
        ...
        # Suppress forward odom when front sensors indicate wall contact — wheels
        # can slip against a wall for 1-2 ticks before stop_motor fires, and
        # integrating those phantom encoder counts is the source of the drift.
        if ds > 0:
            try:
                ds_vals = [s.getValue() for s in self.distance_sensors]
                if len(ds_vals) >= 3 and min(ds_vals[0], ds_vals[2]) < 0.06:
                    return
            except Exception:
                pass
    ...
    def _refresh_map_depth(self, depth_stride=2, max_depth=3.5):
        """Detect floating walls via depth camera and stamp them on the map once.

        Uses the per-pixel downward scan (require_ground_support=True) to
        discriminate floating walls from normal walls at the image level before
        any map projection — the same approach as Maze3.
        """
        ...
        if pts_local is None or pts_local.shape[0] == 0:
            self.occ_map.floating_points = []
            return

        # Distance gate: far detections project poorly and scatter.
        if float(np.median(pts_local[:, 0])) > FLOATING_WALL_MAX_STAMP_DIST:
            self.occ_map.floating_points = []
            return
        ...
        mh, mw = grid.shape
        skip = (GREEN_CARPET, CLOSED, OBSTACLE)

        unique_set = set()
        for mx, my in map_pts:
            mx_i, my_i = int(mx), int(my)
            if 0 <= mx_i < mw and 0 <= my_i < mh:
                if grid[my_i, mx_i] not in skip:
                    unique_set.add((mx_i, my_i))
        ...
        # Already-stamped check: wall registered on a previous pass — don't add more.
        already = unique_set & self.occ_map._depth_obstacle_cells
        if len(already) >= 2:
            self.occ_map.floating_points = [(mx, my, color_tag) for mx, my in already]
            return

        # First detection — dilate to give the wall proper thickness, then stamp.
        r = max(0, int(FLOATING_WALL_THICKNESS))
        thick_set = set()
        for cx, cy in unique_set:
            for dy in range(-r, r + 1):
                for dx in range(-r, r + 1):
                    nx, ny = cx + dx, cy + dy
                    if 0 <= nx < MAP_SIZE and 0 <= ny < MAP_SIZE:
                        if grid[ny, nx] not in skip:
                            thick_set.add((nx, ny))

        self.occ_map._depth_obstacle_cells.update(thick_set)
        for mx, my in thick_set:
            grid[my, mx] = DEPTH_OBSTACLE
        self.occ_map.build_cost_map()
        self.occ_map.floating_points = [(mx, my, color_tag) for mx, my in thick_set]
        ...
        if require_ground_support:
            ...
            for i in np.where(floating_collision_height)[0]:
                ...
                for sv in range(v_row + 1, h):
                    ...
                    if abs(sd - d_ref) > 0.10:
                        if sd < d_ref - 0.10:
                            # A closer surface is occluding the downward view — the
                            # current pixel belongs to a normal wall that sits behind
                            # a floating wall; it has implicit ground support.
                            support_ok[i] = True
                        break
                    y_n2 = (float(sv) - cy) / fy
        ...
        while self.step(self.time_step) != -1 and not self.found_all_2_columns():
            ...
            if debug:
                with self.occ_map.vis_lock:
                    ...
                    col_pts = list(self.occ_map.column_points) if self.occ_map.column_points else []
                    col_pts = [pt for pt in col_pts if isinstance(pt, tuple) and len(pt) >= 3]
                    col_pts = [pt for pt in col_pts if pt[2] not in ((0, 255, 255), (255, 255, 0))]
