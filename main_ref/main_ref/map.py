...
class OccupancyGrid:
    ...
    def __init__(self, robot=None):
        ...
        self.cost_map        = None
        self.dist_map        = None   # distance-transform in cells, updated with cost_map
            ...
            for x, y in cells[:-1]:
                if 0 <= x < MAP_SIZE and 0 <= y < MAP_SIZE:
                    ...
                    if self.log_odds[y, x] < 3.5:
                        self.log_odds[y, x] -= 0.05
            x, y = cells[-1]
            if 0 <= x < MAP_SIZE and 0 <= y < MAP_SIZE:
                if (x, y) not in depth_cells:
                    self.log_odds[y, x] += 1.4
    ...
    def build_cost_map(self, max_dist=None):
        from scipy.ndimage import distance_transform_edt
        if max_dist is None:
            max_dist = COST_MAP_MAX_DIST
        ...
        self.dist_map = dist.astype(np.float32)
